...
func main() {
	...
	const DOMAIN_SUBNAME string = "local"
...
func getDNSRecordIP(domainName string) string {
	pTCPAddr, err := net.ResolveTCPAddr("tcp", "local.itts.io:80")
	if err != nil {
		fmt.Fprintf(os.Stderr, "Err: %s", err.Error())
		return ""
	}

	fmt.Fprintf(os.Stdout, "local.itts.io:80 IP: %s PORT: %d", pTCPAddr.IP.String(), pTCPAddr.Port)
